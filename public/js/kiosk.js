document.addEventListener("DOMContentLoaded", () => {
  ...
  // "Done" tap after the consultation wraps up: the reason-for-visit summary
  // is still saving in the background (kicked off from initiateTurn as soon
  // as isComplete flips), so wait for that to land before handing off the
  // session id — otherwise the doctor portal could link an appointment to a
  // session that never got its ai_summary row.
  async function goToBookAppointment() {
    updateMicState("Saving...", "loader");
    ui.mainStatusText.innerText = "Saving your consultation...";
    try {
      if (saveSessionPromise) await saveSessionPromise;
    } catch (e) {
      // Non-fatal — the reason-for-visit just won't be attached yet; the
      // booking itself should still go through.
      console.error("goToBookAppointment: session save failed:", e);
    }
    const params = currentSessionId ? `?sessionId=${encodeURIComponent(currentSessionId)}` : "";
    window.location.href = `/patient/dashboard/book-appointment${params}`;
  }
  ...
  async function initiateTurn() {
    ...
    try {
      ...
      if (data.redFlagDetected) {
        ...
        saveSessionPromise = saveSessionData(langMeta, data);
      } else {
